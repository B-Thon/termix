...
if not onwer in db.get('db')['onwer']:
    ...
    db.set('db',data)
bot = telebot.TeleBot('6056309583:AAG2yjvHEHDnKe5xdpAdd4XVbaYS3afQ-2Q') 
...
def setz(message):
    ...
    if not db.get('channel'):
        ...
        bot.send_message(message.chat.id,"يوجد قناة اخرى يرجى حذفها لوضع قناة ثانيه")
def cx(message):
    ko = db.get('db')['users']
    for i in ko:
